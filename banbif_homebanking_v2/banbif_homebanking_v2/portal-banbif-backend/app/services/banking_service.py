    ...
    accounts = db.execute(
        text("""
            ...
            WHERE user_id = :user_id
              AND UPPER(TRIM(COALESCE(status, ''))) IN ('ACTIVE', 'ACTIVA')
            ORDER BY id ASC
        """),
        {"user_id": user_id},
    ).fetchall()
    ...
    if account_ids:
        transfer_count = db.execute(
            text("""
                ...
                WHERE account_id = ANY(:account_ids)
                  AND (
                        operation_type ILIKE '%Transferencia%'
                     OR operation_type ILIKE '%PLIN%'
...
def make_transfer(db: Session, user_id, from_account_id, to_account_number: str, amount: Decimal, description: str, token_digital: str):
    ...
    # ----------------------------------------------------------------
    # LIMPIEZA DE CUENTA ORIGEN
    # El frontend puede mandar:
    # 1
    # 1910000010011
    # *******011 - S/ 2810.10
    # ----------------------------------------------------------------
    origin_ref_raw = str(from_account_id or "").strip()

    if not origin_ref_raw:
        raise HTTPException(status_code=400, detail="Selecciona una cuenta origen valida.")

    origin_label = origin_ref_raw.split("-")[0].strip()
    origin_digits = "".join(ch for ch in origin_label if ch.isdigit())
    ...
    origin = db.execute(
        text("""
            ...
            WHERE user_id = :user_id
              AND UPPER(TRIM(COALESCE(status, ''))) IN ('ACTIVE', 'ACTIVA')
              AND (
                    CAST(id AS TEXT) = :origin_raw
                 OR account_number = :origin_raw
                 OR account_number = :origin_label
                 OR REGEXP_REPLACE(COALESCE(account_number, ''), '[^0-9]', '', 'g') = :origin_digits
                 OR (
                        :origin_digits <> ''
                    AND RIGHT(
                            REGEXP_REPLACE(COALESCE(account_number, ''), '[^0-9]', '', 'g'),
                            LENGTH(:origin_digits)
                        ) = :origin_digits
                    )
              )
            ORDER BY id ASC
    ...
    destination_ref = str(to_account_number or "").strip()
    destination_digits = "".join(ch for ch in destination_ref if ch.isdigit())

    if not destination_ref:
        raise HTTPException(status_code=400, detail="Ingresa una cuenta destino o numero PLIN.")

    is_plin = len(destination_digits) == 9

    if is_plin:
        if amount > Decimal("500"):
            raise HTTPException(status_code=400, detail="El limite por operacion para PLIN es S/ 500.00.")

        daily_plin = db.execute(
            text("""
        ...
        if Decimal(str(daily_plin or 0)) + amount > Decimal("2000"):
            raise HTTPException(status_code=400, detail="Superas el limite diario referencial de PLIN de S/ 2000.00.")
    else:
    ...
    if is_plin:
        db.execute(
            text("""
                ...
                WHERE id = :origin_id
            """),
            {"amount": amount, "origin_id": origin_account["id"]},
        )
        ...
        db.execute(
            ...
            """),
            {
                ...
                "description": description or ("PLIN interoperable a " + destination_digits),
                "amount": -amount,
        ...
        return {
            "message": "PLIN realizado correctamente.",
            "amount": float(amount),
            "to_account_number": destination_digits,
        }
    ...
    destination = db.execute(
        text("""
            SELECT *
            FROM public.account
            WHERE UPPER(TRIM(COALESCE(status, ''))) IN ('ACTIVE', 'ACTIVA')
              AND (
                    account_number = :destination_ref
                 OR REGEXP_REPLACE(COALESCE(account_number, ''), '[^0-9]', '', 'g') = :destination_digits
              )
            LIMIT 1
        """),
        {
            "destination_ref": destination_ref,
            "destination_digits": destination_digits,
        },
    ).fetchone()
    ...
    if int(origin_account["id"]) == int(destination_account["id"]):
    ...
    return {
        ...
        "amount": float(amount),
        "to_account_number": destination_account.get("account_number"),
    }
...
def request_credit(
    ...
    # FIX_NORMALIZA_CREDIT_DOCUMENTAL_FINAL
    cat_normal = str(income_category or "").lower()

    if "2da" in cat_normal or "capital" in cat_normal:
        income_category = "Rentas de capital - 2da categoria"
        income_type = "Ingreso variable"
        employment_type = "Rentista independiente"
    elif "1ra" in cat_normal or "alquiler" in cat_normal:
        income_category = "Rentas por alquileres - 1ra categoria"
        income_type = "Ingreso variable"
        employment_type = "Rentista independiente"
    elif "4ta" in cat_normal or "honorarios" in cat_normal:
        income_category = "Independiente con recibos por honorarios - 4ta categoria"
        income_type = "Ingreso variable"
        employment_type = "Independiente"
    else:
        income_category = "Dependiente en planilla - 5ta categoria"
        income_type = "Ingreso fijo"
    ...
    analyst_comment = (
        ...
        f"Ubicacion: {location}. Tipo de ingreso: {income_type}. Categoria de renta: {income_category}. "
    ...
    # FIX_DOCUMENTAL_VALUES_CREDIT
    central_value = "Pendiente de validacion interna"
    if str(bad_credit_history or "").lower() in ("si", "sí", "observado"):
        central_value = "Observado en validacion interna"
    ...
    documental_values = {
        ...
        "tipo_ingreso": income_type,

        "income_category": income_category,
        "categoria_renta": income_category,

        "employment_type": employment_type,
        "situacion_laboral": employment_type,

        "employment_months": employment_months,
        "antiguedad": f"{employment_months} meses",
        ...
        "central_risk": central_value,
        "central_riesgo": central_value,
        "central_de_riesgo": central_value,

        "documents_required": documents,
        "estado_desembolso": "No desembolsado",
        "disbursement_status": "No desembolsado",
    }

    for key, value in documental_values.items():
        if key in cols:
            values[key] = value
    optional_values = {
