...
function renderTechnicalDetail(value, solicitud = {}) {
  ...
  const ubicacion = pick("Ubicacion");
  const tipoIngreso = pick("Tipo ingreso");
  const categoria = pick("Categoria");
  const situacionLaboral = pick("Situacion laboral");
  const antiguedad = pick("Antiguedad meses");
  const estadoCivil = pick("Estado civil");
  const centralRiesgo = pick("Central de riesgo");
  const seguroDesgravamen = pick("Seguro de desgravamen");
